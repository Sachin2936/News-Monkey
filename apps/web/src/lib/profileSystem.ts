...
export function getDailyTasks(): DailyTask[] {
    ...
    const pool = [TASK_POOL[i1], TASK_POOL[i2 === i1 ? (i2 + 1) % TASK_POOL.length : i2]];
    const saved = typeof window !== "undefined" ? (JSON.parse(localStorage.getItem(`dt-v3-${today}`) || "[]") as DailyTask[]) : [];
    return pool.map(t => { const s = saved.find(x => x.id === t.id); return { ...t, progress: s?.progress ?? 0, completed: s?.completed ?? false }; });
}
export function saveDailyTasks(tasks: DailyTask[]) {
    const today = new Date().toDateString();
    if (typeof window !== "undefined") localStorage.setItem(`dt-v3-${today}`, JSON.stringify(tasks));
}

// ── Timeline ───────────────────────────────────────────────────
export function loadTimeline(): TimelineItem[] {
    if (typeof window === "undefined") return [];
    return JSON.parse(localStorage.getItem("profile-timeline-v2") || "[]");
}
export function pushTimeline(item: Omit<TimelineItem, "id">): TimelineItem {
    const full = { ...item, id: Date.now().toString() };
    const list = loadTimeline();
    list.unshift(full);
    if (typeof window !== "undefined") localStorage.setItem("profile-timeline-v2", JSON.stringify(list.slice(0, 30)));
    return full;
}
...
export function loadGoals(): UserGoal[] {
    ...
    return JSON.parse(localStorage.getItem("profile-goals-v2") || JSON.stringify(DEFAULT_GOALS));
}
export function saveGoals(goals: UserGoal[]) {
    if (typeof window !== "undefined") localStorage.setItem("profile-goals-v2", JSON.stringify(goals));
}
...
export function loadUserStats(): UserStats {
    ...
    try { return JSON.parse(localStorage.getItem("ups-v3") || "null") ?? defaultStats(); } catch { return defaultStats(); }
}
export function saveUserStats(s: UserStats) {
    if (typeof window !== "undefined") localStorage.setItem("ups-v3", JSON.stringify(s));
}
